...
public class KeyDerivationServiceImpl : IKeyDerivationServices
{
    ...
    {
        byte[] key = Rfc2898DeriveBytes.Pbkdf2(password, salt, _keyDerivationOptions.KeyDerivationIterations, HashAlgorithmName.SHA512, _cryptographicOptions.ByteLengthOfKey);
        byte[] keyPart = key.Skip(key.Length / 2).ToArray();

        byte[] tag = SHA512.HashData(keyPart);
        byte[] tagPart = tag.Take(_keyDerivationOptions.VerificationTagLength).ToArray();
